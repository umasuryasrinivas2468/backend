...
const CardInput = /*#__PURE__*/(0, _react.forwardRef)(({
  ...
  const inputNumberRef = _react.default.useRef('');
  const sessionRef = _react.default.useRef(cfSession);
  _react.default.useImperativeHandle(ref, () => ({
    doPayment,
    doPaymentWithPaymentSessionId
  }));
  const tdrJsonRef = _react.default.useRef(null);
  const cardBinJsonRef = _react.default.useRef(null);
  const firstEightDigitsRef = _react.default.useRef('');
  ...
  const doPayment = cardInfo => {
    try {
      ...
      const cardPayment = new _cashfreePgApiContract.CFCardPayment(sessionRef.current, cardInfo);
